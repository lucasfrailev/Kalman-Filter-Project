...
VectorXd Tools::CalculateRMSE(const vector<VectorXd> &estimations,
  ...
  */
  ...
  for (int i=0;i<n;i++){
   RSME_ += 1.0 / n * (estimations[i] - ground_truth[i]).transpose() * MatrixXd::Identity(4,4) * (estimations[i] - ground_truth[i]);
  }
...
MatrixXd Tools::CalculateJacobian(const VectorXd& x_state) {
  ...
  if (fabs(sum_of_squares) < 0.0001) {
   ...
   Hj_(2,1) =-x_state(1) * cross_product / (square_root_sum * sum_of_squares);
   Hj_(2,2) =-x_state(0) * cross_product / (square_root_sum * sum_of_squares);
